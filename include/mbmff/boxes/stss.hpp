...
template <>
struct mbmff::basic_box_view<mbmff::box_type::stss> : public mbmff::box_view_base {
...
static_assert(
    !mbmff::basic_box_view<mbmff::box_type::stss>::validate({mbmff::box_header{}, std::span<const std::byte>{}})
);

static_assert(!mbmff::basic_box_view<mbmff::box_type::stss>::validate(
    {mbmff::box_header{},
...
static_assert([] {
    ...
    mbmff::basic_box_view<mbmff::box_type::stss> stss;
    stss.version_ = 0;
    stss.payload = std::span(data);
    auto v = stss.value();
    if (v.size() != 2) {
        return false;
    }
