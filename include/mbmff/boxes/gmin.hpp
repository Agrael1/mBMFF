...
template <>
struct mbmff::basic_box_view<mbmff::box_type::gmin> : public mbmff::box_view_base {
...
static_assert(
    !mbmff::basic_box_view<mbmff::box_type::gmin>::validate({mbmff::box_header{}, std::span<const std::byte>{}})
);

static_assert(!mbmff::basic_box_view<mbmff::box_type::gmin>::validate(
...
static_assert([] {
    ...
    mbmff::basic_box_view<mbmff::box_type::gmin> gmin;
    gmin.version_ = 0;
    gmin.payload = std::span(data);
    auto v = gmin.value();
    return v.graphicsmode == 0 && v.opcolor[0] == 0 && v.opcolor[1] == 0 && v.opcolor[2] == 0 && v.balance == 0;
}());
#endif
