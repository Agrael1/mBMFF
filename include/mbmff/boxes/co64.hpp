...
template <>
struct mbmff::basic_box_view<mbmff::box_type::co64> : public mbmff::box_view_base {
...
static_assert(
    !mbmff::basic_box_view<mbmff::box_type::co64>::validate({mbmff::box_header{}, std::span<const std::byte>{}})
);

static_assert(!mbmff::basic_box_view<mbmff::box_type::co64>::validate(
    {mbmff::box_header{},
...
static_assert([] {
    constexpr std::array<std::byte, 20> data{
        ...
        std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x64},
    };
    mbmff::basic_box_view<mbmff::box_type::co64> co64;
    co64.version_ = 0;
    co64.payload = std::span(data);
    auto v = co64.value();
    if (v.size() != 2) {
        return false;
    }
